...
$module = optional_param('module', 0, PARAM_ALPHANUMEXT); // Group selected.
...
# Output module types
$module_list = array_merge(
    array(0 => get_string('allactivities')),
    block_completion_progress_mods_list($courseid)
);
echo get_string('activity') . '&nbsp;';
echo $OUTPUT->single_select($PAGE->url, 'module', $module_list, $module);
...
$tablecolumns = array('picture', 'fullname', 'lastonline', 'progressbar', 'progress', 'zone');
$tableheaders = array(
    '',
    get_string('fullname'),
    ...
    get_string('zone', 'block_completion_progress'),
);
if ($bulkoperations) {
...
$table->column_style_all('text-align', right_to_left() ? 'right' : 'left');
...
$table->column_style('zone', 'width', '4%');
$table->column_style('zone', 'text-align', 'center');
if ($bulkoperations) {
...
for ($i = $startuser; $i < $enduser; $i++) {
    ...
    $useractivities = block_completion_progress_filter_visibility($activities, $users[$i]->id, $course->id, $exclusions, $module);
...
if ($numberofusers > 0) {
    for ($i = $startdisplay; $i < $enddisplay; $i++) {
        $rowdata = array($rows[$i]['picture'],
            ...
            $rows[$i]['progressbar'], $rows[$i]['progress'], $rows[$i]['zone']);
        if ($bulkoperations) {
